...
$$f_y(\mathbf{y}) = f_x(g^{-1}(\mathbf{y})) |\mathbf{J}_{g^{-1}(\mathbf{y})}|$$
Example: lognormal distribution:
Normal Distribution: 
$$f(x) = \frac{1}{\sqrt{2\pi}\sigma}\text{exp}\Big(-\frac{(x-\mu)^2}{2\sigma^2}\Big)$$
Then if we want $y = exp(x)$, then $J = \frac{d \text{log}(y)}{d y}$ and the density becomes,
...
y <- seq(0, 10, by = 0.1)
plot(y, dlognorm(x=y, 0, 1), xlab = "y", ylab = "lognormal density", type = 'l')
...
plot(p, dlogitnorm(x=p, 0, 1), xlab = "p", 
  ylab = "logitnormal density", type = 'l')
```
